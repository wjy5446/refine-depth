...
def refine_depth_normal_alignment(
    ...
    # === 파라미터 (float32) ===
    sigma_equal  = F32(0.02)
    sigma_plane  = F32(0.02)
    tau_n_plane  = None         # 예: F32(0.95)
    gain_known   = F32(2.0)
    gain_hole    = F32(1.0)
    ...
    if (lambda_equal > 0) or (lambda_plane > 0):
        ...
        for p_sl, q_sl, cand_ok_full in dirs:
            ...
            # NaN과 inf 값을 안전하게 처리 - 더 간단하고 효율적으로
    ...
    # ===== 시스템 조립/해 (float32 유지) =====
    if len(data_buf) == 0:
        out = depth_in.copy().astype(F32)
        out[:] = depth_in
        return out

    data = np.concatenate(data_buf).astype(F32)
    rows = np.concatenate(row_buf).astype(np.int32)
    cols = np.concatenate(col_buf).astype(np.int32)
    b    = np.concatenate(b_buf).astype(F32)

    A = coo_matrix((data, (rows, cols)), shape=(rows.max()+1, N), dtype=F32).tocsr()

    if solver == "cg":
        AtA = (A.T @ A).astype(F32).tocsr()
        Atb = (A.T @ b).astype(F32)
        diag = AtA.diagonal().astype(F32)
        M = diags((F32(1.0) / np.clip(diag, F32(1e-6), None)).astype(F32), dtype=F32)
        z_vec, _ = cg(AtA, Atb, tol=float(tol), maxiter=int(maxiter), M=M)
        z_vec = z_vec.astype(F32, copy=False)
    else:
        sol = lsmr(A, b.astype(F32, copy=False), atol=float(tol), btol=float(tol), maxiter=int(maxiter))
        z_vec = sol[0].astype(F32, copy=False)

    out = depth_in.copy().astype(F32, copy=False)
    out.reshape(-1)[:] = z_vec.astype(F32, copy=False)
    return out
